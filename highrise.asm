;
; ENGINE:	SQueeker Plus. ZX Spectrum beeper engine by utz
; 		Based on Squeeker by zilogat0r
; SONG : 	High and rising.1tm  
; Author:	Shiru
; VZ port:	Dave
;
...
;DE  equ  add counter ch2
;IX  equ  add counter ch3
;IY  equ  add counter ch4
;BC  equ  basefreq ch1-4
;SP  equ  buffer pointer
...
duty4 equ $+1
	...
	and 	BORDER		;7		Masking 33 for bits 1 & 5.
	out ($ff), a		;11		
...
envset1 equ $+1			;update duty envelope pointers
	...
	inc hl
	ld a, (hl)
	cp b			;check for envelope end (b  equ  $80)
	jr z, e2
	ld (duty1), a
	ld (envset1), hl
e2	
...
jrcalc1
	...
	srl d			;if pitch slide is enabled,  de  equ  freq.ch4
	rr e			;else,  de  equ  0
	
	sbc hl, de		;thus,  freq.ch4  equ  freq.ch4 - int(freq.ch4/2)
...
kick1					;27*16*4 + 27*32*4 + 27*64*4 + 27*128*4 + 27*256*4  equ  53568,  + 20*33  equ  53568 -> -147, 4 loops -> AF'  equ  $6D
	ds 4, $10
	ds 4, $20
	ds 4, $40
	ds 4, $80
